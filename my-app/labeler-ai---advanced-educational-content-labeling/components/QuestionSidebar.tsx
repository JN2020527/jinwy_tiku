    ...
    <aside className="w-[320px] flex flex-col border-r border-border-light bg-white shrink-0 z-10 h-full">
      <div className="p-3 border-b border-border-light space-y-2 bg-gray-50/30">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <span className="material-symbols-outlined absolute right-0.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none text-[16px]">arrow_drop_down</span>
          </div>
          ...
          return (
            <div 
              ...
              className={`group p-3 border-b border-slate-100 flex gap-3 cursor-pointer transition-all ${
                isActive ? 'bg-blue-50/60 border-l-[3px] border-l-primary' : 'bg-white hover:bg-slate-50'
              }`}
            >
              <div className="pt-1 flex items-start" onClick={(e) => { e.stopPropagation(); onToggleSelection(q.id); }}>
                <input 
                  type="checkbox" 
                  checked={isSelected}
                  onChange={() => {}}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between items-center mb-1.5">
                  <span className={`font-mono text-[10px] tracking-wide ${isActive ? 'text-primary font-bold' : 'text-slate-400 font-medium'}`}>
                    {q.code}
                  </span>
                  {q.status === 'Completed' && <span className="material-symbols-outlined text-emerald-500 text-[16px]">check_circle</span>}
                  {q.status === 'AI Suggested' && <span className="material-symbols-outlined text-purple-500 text-[16px] animate-pulse">auto_awesome</span>}
                  {q.status === 'Active' && (
                  ...
                  {q.status === 'Pending' && <span className="material-symbols-outlined text-slate-300 text-[16px]">schedule</span>}
                </div>
                <p className={`text-xs line-clamp-2 mb-2 ${isActive ? 'text-slate-900 font-semibold' : 'text-slate-600 font-medium'}`}>
                  {q.title}
                </p>
                <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase tracking-wide border ${
                    isActive ? 'bg-white text-primary border-primary/20' : 'bg-slate-100 text-slate-500 border-transparent'
                  }`}>
                    {q.tags[0]}
                  </span>
                </div>
      ...
      <div className="p-2 border-t border-border-light bg-white text-center">
        <button className="text-[10px] font-bold uppercase tracking-wider text-slate-500 hover:text-primary hover:bg-slate-50 py-2 w-full rounded transition-colors flex items-center justify-center gap-1">
          <span className="material-symbols-outlined text-[14px]">{selectedIds.size > 0 ? 'checklist' : 'add'}</span> 
          {selectedIds.size > 0 ? `${selectedIds.size} Selected` : 'Load More Questions'}
        </button>
      </div>
    </aside>
  );
