  ...
  const renderNode = (node: KnowledgeNode, depth: number = 0) => {
    ...
    return (
      <div key={node.id} className="select-none">
        <div className={`flex items-center gap-2 p-1.5 rounded hover:bg-slate-100 cursor-pointer text-slate-700 font-medium`}>
          {isFolder ? (
          ...
          {node.icon && (
            <span className="material-symbols-outlined text-purple-500 text-[18px]">{node.icon}</span>
          )}
          ...
          {!isFolder && (
            <div className="relative flex items-center">
              ...
              />
              <span className="material-symbols-outlined absolute text-white text-xs opacity-0 peer-checked:opacity-100 pointer-events-none inset-0 flex items-center justify-center">check</span>
            </div>
          )}

          <span className={`text-sm ${!isFolder && node.aiMatch ? 'text-primary font-semibold' : ''}`}>
            {node.label}
          </span>
    ...
    <>
      <div className="p-4 border-b border-border-light bg-white">
        <div className="flex justify-between items-center mb-3">
          ...
          <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full uppercase tracking-wider">AI Assisted</span>
        </div>
      ...
      <div className="p-5 border-t border-border-light bg-white space-y-5">
        <div>
          <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Difficulty Assessment</label>
        ...
        <div className="pt-2">
          <button className="w-full bg-primary hover:bg-blue-600 text-white font-bold h-11 rounded-lg shadow-lg shadow-blue-500/30 flex items-center justify-between px-6 transition-all transform active:scale-[0.99]">
            ...
            <div className="flex items-center gap-2">
              <kbd className="hidden md:inline-block font-sans font-medium text-[10px] bg-blue-500 text-white px-1.5 py-0.5 rounded border border-blue-400/50">⌘ S</kbd>
