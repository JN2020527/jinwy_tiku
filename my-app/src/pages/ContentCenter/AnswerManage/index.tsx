...
const AnswerManage: React.FC = () => {
          ...
          record.answerType === 'video' && (
            <a
              ...
              onClick={() => {
                ...
                setShowVideoSummary(
                  !!(record.videoSummary && record.videoSummary.length > 0),
                );
  ...
  const handleBatchDownload = () => {
    ...
    if (!isDirectoryEnabled) {
      ...
      resourceQRs = answerList.filter(
        (item) => !item.directoryId && !!item.qrCodeUrl,
      );
      directoryQRs = [];
    } else {
      ...
      const targetDirIds = [
        selectedDirectoryKey,
        ...getDescendantIds(selectedDirectoryKey, directoryList),
      ];

      // Collect Directories
      directoryQRs = directoryList.filter(
        (d) => targetDirIds.includes(d.id) && !!d.qrCodeUrl,
      );
      ...
      resourceQRs = answerList.filter(
        (item) =>
          item.directoryId &&
          targetDirIds.includes(item.directoryId) &&
          !!item.qrCodeUrl,
      );
    }
              ...
              prev.map((item) => {
                if (item.id === currentEditingItem.id) {
                  return {
                    ...
                    name:
                      values.title +
                      (item.type ? `.${item.type.toLowerCase()}` : ''), // Reconstruct name
