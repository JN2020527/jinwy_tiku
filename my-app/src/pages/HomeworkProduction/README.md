...
- **三栏工作台**（新建/编辑共用）：左栏知识点树单选（选中父节点按全部后代叶子筛选并展开子树，支持树内搜索）；中栏筛选区（来源/题型/难度三行，每行单选/多选切换 + 顶部重置）、最新/最热排序、结果内搜索、分页（10 题/页）、题卡（来源/题型/难度/年份、题干、选项、答案解析）与唯一主操作「加入作业/移除」；右栏为默认收起的「作业篮」，收起态显示已选数量，展开后查看并维护试题明细（序号、上移、下移、移除、清空确认、0/60 计数）。作业篮只是已选列表的界面名称，仍直接构成作业本体，不是独立中间态。
...
- **预览**：参考教师端作业试卷的连续纸张排版，标题下提供班级/姓名纸面占位，题号与题干同行、选项桌面端双栏；答案解析由页面顶部按钮区的「显示答案解析 / 隐藏答案解析」统一控制全部显示或隐藏，显示时以灰底分区呈现。学科与正式状态由页面标题区展示，试题内容按 ID 动态读取，缺题展示明确提示。
