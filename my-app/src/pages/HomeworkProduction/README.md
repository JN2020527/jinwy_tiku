...
- **保存**：Modal 填写名称（必填），调用 `saveHomework`；名称冲突错误落在 name 字段；编辑模式且影响计数（挂载/平台模板/教师任务）非零时，保存前用紧凑三列网格确认。
- **未保存离开**：返回按钮与 `history.block` 均轻确认（Modal 确认后离开，不保留现场），`beforeunload` 兜底；保存成功导航前解除阻止。dirty = 试题列表顺序变化或名称相对已保存状态变化。
- **预览**：连续成品文档式排版（名称、学科、正式状态、按序题号/题干/选项、答案解析折叠），顶部返回/编辑，无结构卡片与调序操作；试题内容按 ID 动态读取，预览即最新版本；缺题展示明确缺失提示。
- **不含**教师端专属操作（相似题/纠错/收藏/下载）与复制/另存为入口。
- 编辑和新建发生题目列表变更时 dirty；编辑名称相对保存状态变化也算 dirty（名称在页内输入，保存弹窗预填，弹窗应用方案见 index.tsx `handleSaveConfirm`）。
...
- **名称弹窗应用方案**：页内名称输入 + 保存弹窗预填，取消弹窗不改变页内名称；由 UI 决定，不影响产品规则（PRD §8 允许）。
