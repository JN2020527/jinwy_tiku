...
- **选题工作台**（新建/编辑共用）：左栏知识点树单选（选中父节点按全部后代叶子筛选并展开子树，支持树内搜索）；中栏筛选区（来源/题型/难度三行，每行单选/多选切换 + 顶部重置）、最新/最热排序、结果内搜索、分页（10 题/页）、题卡（来源/题型/难度/年份、题干、选项、答案解析）与唯一主操作「加入作业篮/移出作业篮」。右侧默认只显示稳定的作业篮入口与 0/60 计数，点击后以抽屉按题型分组展示完整题干和选项；组序取题型首次出现顺序，组内保留平铺顺序并支持组内上移、下移，另可移除或清空。已选题卡会直接标明其在作业篮中的顺序。作业篮只是已选列表的界面名称，仍直接构成作业本体，不是独立中间态。
- **限制与提示文案**：去重（卡片按钮切换为「移出作业篮」+ 加入逻辑防重）；60 道上限提示「作业最多可添加 60 道题」；空列表保存提示「至少加入 1 道题」。
...
- **预览**：参考教师端作业试卷的连续纸张排版，按题型自动分段，组序和组内顺序与作业篮一致；标题下提供班级/姓名纸面占位，题号与题干同行、选项桌面端双栏。答案解析由页面顶部按钮区的「显示答案解析 / 隐藏答案解析」统一控制全部显示或隐藏，显示时以灰底分区呈现。学科与正式状态由页面标题区展示，试题内容按 ID 动态读取，缺题展示明确提示。
...
| AC-04 加入作业篮/切换移出 | `Workbench.tsx` 题卡唯一主操作 |
